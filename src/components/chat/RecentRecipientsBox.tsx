...
export function RecentRecipientsBox({ 
  ...
  return (
    <div className={cn(
      "mt-3 rounded-xl overflow-hidden border border-border/50 bg-gradient-to-br from-primary/5 to-secondary/5",
      className
    )}>
      {/* Header */}
      <div className="px-4 py-2.5 bg-primary/10 border-b border-border/30">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium text-foreground">Recent Recipients</span>
        </div>
      </div>
      ...
      <div className="divide-y divide-border/30">
        {recipients.map((recipient, index) => (
          <button
            ...
            className="w-full px-4 py-3 flex items-center gap-3 hover:bg-primary/5 transition-colors text-left group"
          >
            {/* Number Badge */}
      ...
      <div className="px-4 py-2 bg-muted/30 border-t border-border/30">
        <p className="text-xs text-muted-foreground text-center">
          Click to send to this recipient, or type a new name
        </p>
      </div>
    </div>
  );
